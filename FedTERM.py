...
for epoch in range(EPOCHS):
    ...
    for client, supervision_t in zip(CLIENTS, CLIENTS_SUPERVISION):
        ...
        if client != 'Interobs' and client != 'Lung1':
            acc_, loss_ = train_model(epoch, training_clients[client], optimizers[client], device, nets[client],
                                      nets['global'], ema_model=ema_net, acc=acc_train[client], supervision_type=supervision_t,loss=loss_train[client],
    ...
    variances = []
    for client in CLIENTS:
        history = CLIENTS_HISTORY[client]
        if len(history) > 1:
    ...
    if epoch < 150:
        ...
        for idx, s in zip(labeled_indices, scores):
            WEIGHTS_DATA[idx] = s / scores_sum
        WEIGHTS = WEIGHTS_DATA
    else:
        unlabeled_weight = 0.04
        unlabeled_indices = []
        labeled_indices = []
        for i in range(len(CLIENTS)):
            if CLIENTS_SUPERVISION[i] == "unlabeled":
                unlabeled_indices.append(i)
                WEIGHTS_DATA[i] = unlabeled_weight
            else:
                labeled_indices.append(i)
        remaining_weight = 1 - unlabeled_weight * len(unlabeled_indices)
        scores = [1 / (variances[i] + epsilon) for i in labeled_indices]
        scores_sum = sum(scores)
        for idx, s in zip(labeled_indices, scores):
            WEIGHTS_DATA[idx] = (s / scores_sum) * remaining_weight
        WEIGHTS = WEIGHTS_DATA
