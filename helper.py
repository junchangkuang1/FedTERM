...
def train_model(epoch, trainloader, optimizer_stu, device, net_stu, net_glb, ema_model=None,
    ...
    """
    Training function for federated semi-supervised segmentation with CPL pseudo-labels.

    Args:
        cluster_centers_cross_client: list or tensor of representative cluster centers from other clients,
                                      shape: [K, C], K = total cluster centers, C = feature dimension
        sigma: temperature parameter for converting distances into pseudo-label probabilities
    """
    net_stu.train()
    ...
    for i, batch in enumerate(trainloader):
        ...
        if supervision_type == 'labeled':
            ...
            # Semi-supervised with CPL
            # 1. Extract high-level semantic features from projection layer
            features = net_stu.extract_features(images)  # [B, C, H, W]
            B, C, H, W = features.shape
            features_flat = features.view(B, C, -1).permute(0, 2, 1).contiguous()  # [B, H*W, C]

            # 2. Compute distance to cross-client cluster centers
            # cluster_centers_cross_client: [K, C]
            cluster_centers = cluster_centers_cross_client.to(device=device)
            distances = torch.cdist(features_flat, cluster_centers)  # [B, H*W, K]

            # 3. Generate probabilistic CPL pseudo-labels
            pseudo_probs = torch.softmax(-distances / sigma, dim=-1)  # [B, H*W, K]
            # Optionally convert back to spatial shape
            pseudo_label_cpl = pseudo_probs.max(dim=-1)[0].view(B, H, W)  # [B, H, W]

            # 4. Compute Dice loss between student prediction and CPL pseudo-label
            diceloss = (1 - dice_coeff(pseudo_label_cpl, masks_pred))[0]

            # 5. Local entropy regularization
